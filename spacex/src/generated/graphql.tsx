...
export type CompanyInfoQuery = (
  ...
  & { info?: Maybe<(
    { __typename?: 'Info' }
    & Pick<Info, 'name' | 'founder' | 'ceo' | 'cto' | 'coo' | 'launch_sites' | 'test_sites' | 'valuation' | 'vehicles' | 'employees' | 'founded' | 'summary'>
    & { headquarters?: Maybe<(
      { __typename?: 'Headquarters' }
      & Pick<Headquarters, 'address' | 'city' | 'state'>
    )> }
  )> }
);
...
export type MissionQueryVariables = Exact<{ [key: string]: never; }>;
...
export type MissionQuery = (
  ...
  & { missions?: Maybe<Array<Maybe<(
    { __typename?: 'Mission' }
    & Pick<Mission, 'mission_id' | 'mission_name' | 'wikipedia' | 'website' | 'description'>
  )>>> }
);
...
export type RocketsQuery = (
  ...
  & { rockets?: Maybe<Array<Maybe<(
    { __typename?: 'Rocket' }
    & Pick<Rocket, 'id' | 'rocket_name' | 'active' | 'stages' | 'boosters' | 'cost_per_launch' | 'first_flight' | 'country' | 'company' | 'flickr_images' | 'description'>
  )>>> }
);
